...
class Administrators extends CI_Controller
{
    ...
    // function printRaportSantri()
    // {
    //     $data['tahun'] = $this->m_admin->raportTahun();
    //     $data['kelas'] = $this->m_admin->printSelect();

    //     $data['sidebar_role'] = "admkinistrator";
    //     $data['tittle'] = "Cetak Raport";
    //     $this->load->view("tpq/header", $data);
    //     $this->load->view("tpq/sidebar_role", $data);
    //     $this->load->view("tpq/administrator/raport_print", $data);
    //     $this->load->view("tpq/footer");
    // }
    // form piliha cetak roaport ummi
    function raportUmmi()
    {
        $data['tahun'] = $this->m_admin->raportTahun();
        $data['kelas'] = $this->m_admin->printSelect('Ummi');
        $data['tahun'] = date('Y');

        // print_r($data['tahun']+3);

        $data['sidebar_role'] = "administrator";
        $data['tittle'] = "Cetak Raport Ummi";
        $this->load->view("tpq/header", $data);
        $this->load->view("tpq/sidebar_role", $data);
        $this->load->view("tpq/administrator/raportUmmiForm", $data);
        $this->load->view("tpq/footer");
    }
    // form pilih cetak raport diniah
    function raportDiniah()
    {
        $data['tahun'] = $this->m_admin->raportTahun();
        $data['kelas'] = $this->m_admin->printSelect('Diniah');

        $data['sidebar_role'] = "administrator";
        $data['tittle'] = "Cetak Raport Diniah";
        $this->load->view("tpq/header", $data);
        $this->load->view("tpq/sidebar_role", $data);
        $this->load->view("tpq/administrator/raportDiniahForm", $data);
        $this->load->view("tpq/footer");
    }

    // layout print raport
    function printRaportDiniah()
    {
        $idKelas = $this->input->post('id_kelas');

        $data['raportDiniah'] = $this->m_admin->mprintRaportDiniah($idKelas);
        $data['semester'] = strtoupper($this->m_admin->semester());
        // $data["waliKelas"] = $this->m_admin->findWalikelas(31);
        $data['totalSantri'] = $this->m_admin->totalSantriDiniah();
        $data["ketua"] = $this->m_admin->ketuaTpq(); // ketua tpq
        
        // print_r($data['raportDiniah']);
        $this->load->view("tpq/administrator/raport_diniah", $data);
    }


    function printRaportUmmi()
    {
        $idKelas = $this->input->post("id_kelas");
        $data["waliKelas"] = $this->m_admin->findWalikelas(31);
        $data["ketua"] = $this->m_admin->ketuaTpq(); // ketua tpq
        $data['raportUmmi'] = $this->m_admin->mprintRaportUmmi($idKelas);
        $data['semester'] = strtoupper($this->m_admin->semester());

        $data['totalSantri'] = $this->m_admin->totalSantriUmmi();
        // print_r($idKelas);
        ...
        // print_r($data['raportUmmi']);

        $this->load->view("tpq/administrator/raport_ummi", $data);
    }
}
